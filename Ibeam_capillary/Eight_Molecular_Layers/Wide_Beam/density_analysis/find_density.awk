...
{
	if ($2=="TIMESTEP") {
		if (start==1 && N>0) { print YBIN "\t" N "\t" XMIN "\t" XMAX "\t" XMAX-XMIN+3.0 "\t" (XMAX-XMIN+3.0)*YBIN_WIDTH*ZBIN_WIDTH "\t" N/((XMAX-XMIN+3.0)*YBIN_WIDTH*ZBIN_WIDTH) ; exit } ; 
		if (start==1 && N==0) { print YBIN "\t" N "\t" 0 "\t" 0 "\t" 0 "\t" 0 "\t" 0 ; exit } ;
		start = 0 ; t = 1 ; a = 0 
	}
